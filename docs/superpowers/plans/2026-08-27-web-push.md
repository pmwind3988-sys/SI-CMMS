...
- **Migration number is 0039.** `0038_accept_decline_fanout_and_clear_notifications.sql` is the last on disk. A migration cannot be inserted between two existing ones — the CLI orders by filename and the version parser takes digits only.
...
| `app/supabase/migrations/0039_web_push.sql` | **create** — `push_subscriptions`, `notifications.pushed_at`, the two register RPCs, the guard amendment, `si_enqueue_push()`, the trigger, `si_push_retry_sweep()`, the cron entry. |
...
## Task 1: Migration 0039 — schema, RPCs, trigger, sweep

**Files:**
- Create: `app/supabase/migrations/0039_web_push.sql`
...
```sql
-- ===========================================================================
-- 0039 — Web Push: an alert that arrives when the app is not running
-- ===========================================================================
...
Expected: `env:which` reports **test**, then `Applying migration 0039_web_push.sql...` and `Finished supabase db push.`

**If this fails, stop and read the error rather than editing and re-pushing.** To re-run an edited migration on test: `npx supabase migration repair --status reverted 0039` then `db push` again.
...
```bash
git add app/supabase/migrations/0039_web_push.sql app/src/lib/database.types.ts
git commit -m "0039: the database can hand a notification to a sender, and knows if it arrived"
```
 ...
 * registered. Called by si_enqueue_push() (0039) on insert, and again by
 ...
 * retry sweep re-sending forever, and it only works because 0039 amended
 ...
 * by supabase/functions/push-notify (migration 0039).
```
 ...
 *
 * The browser half of migration 0039. A PushSubscription identifies a BROWSER,
