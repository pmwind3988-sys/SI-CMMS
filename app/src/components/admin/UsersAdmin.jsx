...
export default function UsersAdmin() {
  ...
  const pageSize = useAutoPageSize([tableRef, cardsRef], { min: 5, ready: !!users, signature: filtered.length });
    ...
    <div className="max-w-6xl">
      ...
      <Card className="hidden overflow-hidden lg:block">
        <div className="flex items-center px-4 py-2.5 bg-canvas text-[11.5px] font-bold text-ink-soft uppercase tracking-wide">
          ...
          <div className="w-[300px] text-right">Actions</div>
        </div>
        ...
        {pager.visible.map((u, i) => (
          ...
          >
            ...
            {/* flex-wrap so the labelled buttons drop to a second line as whole
                buttons if a name pushes them past the column, rather than the
                text wrapping inside a squished button — the tall "Send reset
                link" box that used to appear here. */}
            <div className="w-[300px] flex flex-wrap items-center justify-end gap-1.5">
              <UserActions
                user={u}
                me={me}
                setPanel={setPanel}
...
function UserActions({ user, me, setPanel, onToggleStatus }) {
  ...
  return (
    <>
      {/* Superuser only, for anyone but yourself. Setting a temporary password
          is the sole password route now — the emailed reset link was removed,
          so an Administrator whose subordinate needs a new password asks the
          Superuser to set one here. An Administrator who could set a
          subordinate's password would hold that person's credential, which is
          why the button is absent for them rather than present and refused. */}
      {maySetPassword && (
        <Button
          size="sm"
          variant="ghost"
